...
exports.UploadFile= async(req, res) => {
  //  console.log("inside")
   ...
   const{postname, title,description,department,under,academiclevel,status,state} = req.body;
...
let newUpload = new Upload({username:uploader.username, postname, file:fileName, title, description,department,under,academiclevel,status,state, uploader,institute:uploader.institute})
...
exports.FilterpostList = async(req, res) => { 
  ...
  departments = departments ? { department: { $in : JSON.parse(departments)}} : {} 
  tags= tags ?{tag:{ $in : JSON.parse(tags)}} :{} 
  date = date ? { date: {$gt: new Date(date)}} : {}; 
  const post = await Upload.find({state: "post", ...departments, ...date , ...tags}); 
   res.json({post}) 
}
...
exports.likePost  = async (req, res)=> {
  ...
  if(!upload) {
    ...
    if(student.like1 == null){
      ...
      student.like1 =`localhost:${process.env.APP_PORT}/resource/`+upload.file
    }
    else if(student.like2 == null ){

      student.like2 = `localhost:${process.env.APP_PORT}/resource/`+ upload.file
    }
    else if(student.like3 == null){
      student.like3 = `localhost:${process.env.APP_PORT}/resource/`+upload.file
      
    }
    else if(student.like4 == null){
      student.like4 = `localhost:${process.env.APP_PORT}/resource/`+upload.file
      
    }
    else if(student.like5 ==null){
      student.like5 = `localhost:${process.env.APP_PORT}/resource/`+upload.file
...
exports.search = async (req, res) =>{
  ...
  if(req.query.key == "department"){
    ...
    try{
      const admins  = await Admin.find({ $text: {$search:req.query.val}})
      console.log(admins)
      for(var i =0; i<admins.length;i++){ // TODO: change array to single admin
        console.log(admins[i])
        if(admins[i].name == req.query.val){
          console.log("id: " , admins[i]._id)
          const uploads = await Upload.find({institute:admins[i]._id})
          console.log(uploads)  
          return res.json({
            success: true,
            data: {
